...
public class BoardGenerator : MonoBehaviour
{
    ...
    {
        while (target.position.z + generationDistance > lastZOffset)
        {
            lastZOffset += generationOffset;
            obstacles.Add(GenerateObstacle(lastZOffset));
        }
    ...
    {
        Vector3 spawnPosition = new Vector3(Random.Range(boardBorders[0], boardBorders[1]), target.position.y, lastZOffset);
